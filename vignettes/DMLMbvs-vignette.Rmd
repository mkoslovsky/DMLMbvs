...
# Simulate Data
Having installed and loaded the `DMLMbvs` package into the R environment, generate a simulated data set using the `simualate_DMLM()` function. 
...
# Run model
To run the model, simply imput the data to the `dm_lm_bvs_R()` command. 
...
For inference, the `selected` function can be used to generate plots of the MPPIs for both levels of the model as with as plots of the number of terms active in the model at each MCMC iteration. The latter can be used to help assess convergence of the model. To obtain these plots, simply run 
