...
List dm_lm_bvs(
  ...
  for( int iter = 0; iter < iterations; ++iter ){
    ...
    between_phi_zeta = help::between_phi_zeta_augment_update_cpp( x, temp_zeta, temp_alpha, temp_phi, temp_psi, temp_loggamma, temp_cc, sigma2_phi, a, b );
    ...
    // Update within
    phi_out = help::within_phi_augment_cpp( x, temp_zeta, temp_alpha, temp_phi, temp_psi, temp_loggamma, temp_cc, sigma2_phi );
    temp_phi =  as<arma::mat>( phi_out[ 0 ] );
    temp_loggamma = as<arma::mat>( phi_out[ 1 ] );

    // Update alphas
    alpha_out = help::alpha_update_augment_cpp( temp_loggamma, temp_cc, temp_alpha, temp_phi, temp_psi, sigma2_alpha );
